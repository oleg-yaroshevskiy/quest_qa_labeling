...
TARGETS = ['question_score', 'question_view_count', 'question_favorite_count',
           'answer_score', 'answers_count', 'is_answer_accepted']

# change paths here
PATH_TO_DATA = Path('/home/kashn500/data/qa_quest/')

stackx_large_path = PATH_TO_DATA / 'stackx_large_sop_aux'
checkpoint_dir = PATH_TO_DATA / 'stackx_large_cased_finetuned'
stackx_data = pd.read_csv(PATH_TO_DATA / 'so_sx_7700k_posts.csv.gz')
...
# subsampling
stackx_data = stackx_data.sample(n=LEN_TO_SAMPLE, random_state=SEED)
...
class QuestMLMDataset(QuestDataset):
    def __init__(self, data_df, tokenizer, max_seg_length=512, answer_ratio=0.5,
        ...
        super(QuestMLMDataset, self).__init__(data_df=data_df, tokenizer=tokenizer, max_seg_length=max_seg_length,
                                              ...
                                              body_col=body_col, answer_col=answer_col)
    ...
    def _mask_tokens(self, inputs, masked_random_replace_prob=0.2):
        ...
        labels[~masked_indices] = self.non_masked_idx  # We only compute loss on masked tokens

        # 80% of the time, we replace masked input tokens with tokenizer.mask_token ([MASK])
        indices_replaced = torch.bernoulli(
            torch.full(labels.shape, 1 - masked_random_replace_prob)).bool() & masked_indices
...
def normalize_targets(stackx_data):
    encoded = []
...
stackx_data = normalize_targets(stackx_data)

train_df, test_df = train_test_split(stackx_data, test_size=10000 / LEN_TO_SAMPLE,
                                     random_state=SEED)
...
config = BertConfig(str(stackx_large_path / 'stackx-large-cased-config.json'))
model = BertPretrain(config, len(TARGETS))

state_dict = torch.load(stackx_large_path / 'pytorch_model.bin')
...
class Spearman(EpochMetric):
    ...
    def get_metric(self):
        corr = spearman_metric(np.vstack(self.targets), np.vstack(self.preds),
                               return_scores=True)
...
class MaskLMCrossEntropyLoss(torch.nn.CrossEntropyLoss):
    def forward(self, logits, targets):
        ...
        loss = super(MaskLMCrossEntropyLoss, self).forward(logits.view(n_samples, -1), targets.view(n_samples))
        return loss


class SOPCrossEntropyLoss(torch.nn.CrossEntropyLoss):
    def forward(self, logits, targets):
        loss = super(SOPCrossEntropyLoss, self).forward(logits.view(-1, 2), targets.view(-1))
        return loss


class PretrainingLoss(torch.nn.Module):
    def __init__(self, targets_alpha=1.):
    ...
    def forward(self, logits, targets):
        return self.mlm_loss(logits[0], targets[0]) \
...
checkpoint_name = 'stackx_large_with_aux_ep_{epoch:03}_val_ppl_{val_mlm_ppl:.2f}_val_spearman_{val_spearman:.2f}.pth'
