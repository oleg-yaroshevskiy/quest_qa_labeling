...
class LosswiseSessionHandler:
    def __init__(self, api_key, tag='', params=None):
        ...
        self._session = losswise.Session(tag=tag, params=params if params is not None else {}, track_git=False)
        self._graphs = dict()
...
class CSVParamLogger(CSVLogger):
    ...
    def __init__(self, filename, *, batch_granularity=False, separator=',', append=False, extra_metrics=[]):
        super(CSVParamLogger, self).__init__(filename, batch_granularity=batch_granularity, separator=separator,
                                             append=append)
