...
stackx_large_path = PATH_TO_DATA / 'stackx-large-cased'
checkpoint_dir = PATH_TO_DATA / 'stackx_large_sop_aux'
stackx_data = pd.read_csv(PATH_TO_DATA / 'so_sx_10k_posts.csv')
...
train_df, test_df = train_test_split(stackx_data, test_size=10000 / len(stackx_data),
                                     random_state=SEED)
...
train_dataset = QuestMLMDataset(train_df, aug_tokenizer, target_cols=TARGETS)
...
state_dict = torch.load(stackx_large_path / 'stackx-large-cased.pth')
