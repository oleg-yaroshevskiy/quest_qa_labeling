...
def rareness_split(train_df, least_representative_cols=('question_type_spelling',)):
    ...
    for col in least_representative_cols:
        most_common_value = train_df[col].value_counts().sort_values(ascending=False).index[0]
...
def stratified_fold_split_for_common(common_samples, n_splits=5, interaction_order=1, random_state=42,
                                     agg_func=pd.Series.mode):
    body_encoder = LabelEncoder()
    common_samples['group_id'] = body_encoder.fit_transform(common_samples['question_body'].astype(str))
    ...
    with Pool(cpu_count()) as pool:
        aggregated_common_ordinals = list(
            tqdm(
                pool.imap(functools.partial(aggregate_ordinals, agg_func=agg_func), common_groups),
                total=len(common_groups), desc='Aggregate ordinals over groups'
            )
        )
    aggregated_common_ordinals = pd.concat(aggregated_common_ordinals, axis=1).transpose()
    aggregated_common_ordinals.index.rename('group_id', inplace=True)

    k_fold = IterativeStratification(n_splits=n_splits, order=interaction_order, random_state=random_state)
    folds_common = []
    for _, fold_groups in k_fold.split(aggregated_common_ordinals, aggregated_common_ordinals):
...
def stratified_fold_split_for_rare(rare_samples, n_splits=5, interaction_order=1, random_state=42,
                                   least_representative_cols=('question_type_spelling',)):
    rare_ordinals = transform_target_columns_to_ordinals(rare_samples[list(least_representative_cols)])
