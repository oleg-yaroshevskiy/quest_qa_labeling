...
tokenizer = BertTokenizer(str(checkpoint_dir / 'stackx-base-cased-vocab.txt'),
                          do_lower_case=False)


def get_model():
    config = BertConfig.from_json_file(str(checkpoint_dir / 'stackx-base-cased-config.json'))
    config.__dict__['num_labels'] = len(ALL_TARGETS)

    model = BertForQuestRegression(config)
    state_dict = torch.load(
        checkpoint_dir / 'stackx_base_with_aux_ep_080_val_perplexity_4.39_val_spearman_0.34.pth',
        map_location='cpu'
    )
    state_dict = {
        name.replace('LayerNorm.gamma', 'LayerNorm.weight').replace('LayerNorm.beta', 'LayerNorm.bias'): weight
...
for fold_idx in range(N_FOLDS):
    ...
    model = get_model()
    trainer = Model(model, Adam(model.parameters(), lr=3e-5),
                    'bce_with_logits', epoch_metrics=[spearman])
    trainer.to('cuda')

    checkpoint_name = 'stackx_with_aux_80_fold_' + \
                      str(fold_idx) + \
    ...
    callbacks = [
        ...
        CSVParamLogger(str(out_checkpoint_dir / 'training_log.csv'),
                       append=fold_idx != 0, extra_metrics=ALL_TARGETS)
    ]
