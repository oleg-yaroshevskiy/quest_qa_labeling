...
class TestQuestDataset(QuestDataset):
    def __init__(self, data_df, tokenizer, max_seg_length=256, answer_ratio=0.5, title_ratio=0.5,
                 use_title=True, use_body=True, use_answer=True,
                 title_col='question_title', body_col='question_body', answer_col='answer'):
...
class QuestSiameseDataset(QuestDataset):
    ...
    def __getitem__(self, index):
        ...
        input_ids, attention_mask, token_type_ids = self._process(title=title, body=body)
        question_inputs = list(map(torch.LongTensor, [input_ids, attention_mask, token_type_ids]))

        input_ids, attention_mask, token_type_ids = self._process(body=answer)
        answer_inputs = list(map(torch.LongTensor, [input_ids, attention_mask, token_type_ids]))
