...
class QuestDataset(Dataset):
    ...
    def __init__(self, data_df, tokenizer, max_seg_length=256, target_cols='all_targets',
                 answer_ratio=0.5, title_ratio=0.5,
        ...
        self.target_cols = QUESTION_TARGETS + ANSWER_TARGETS if target_cols is 'all_targets' else target_cols
    ...
    def _encode_segments(self, *text_segments: List[Text]) -> List[List[int]]:
        ...
        return [
            self.tokenizer.encode(txt, max_length=self.max_seg_length, add_special_tokens=False)
            if txt is not None else [] for txt in text_segments
        ]

    def _process(self, title=None, body=None, answer=None):
        input_ids, attention_mask, token_type_ids = self._prepare_features(title, body, answer)

        input_ids = self._pad_and_truncate(input_ids, pad_value=self.tokenizer.pad_token_id)
        token_type_ids = self._pad_and_truncate(token_type_ids, pad_value=token_type_ids[-1])
    ...
    @staticmethod
    def _balance_segments(first_segment_length, second_segment_length, second_ratio, max_length):
        first_segment_length = min(first_segment_length,
                                   (1 - second_ratio) * max_length +
                                   max(second_ratio * max_length - second_segment_length, 0))

        second_segment_length = min(second_segment_length,
    ...
    def _prepare_features(self, title, body, answer):
        title_input_ids, body_input_ids, answer_input_ids = self._encode_segments(title, body, answer)

        title_length = len(title_input_ids)
        body_length = len(body_input_ids)
        answer_length = len(answer_input_ids)

        question_length, answer_length = self._balance_segments(title_length + body_length, answer_length,
                                                                self.answer_ratio, self.max_seg_length)

        title_length, body_length = self._balance_segments(title_length, body_length, self.title_ratio, question_length)

        # TODO: generalize this
        question_input_ids = body_input_ids[:body_length]
        if title_length > 0:
            question_input_ids = title_input_ids[:title_length] + [self.tokenizer.sep_token_id] + question_input_ids
        answer_input_ids = answer_input_ids[:answer_length]

        input_ids = self.tokenizer.build_inputs_with_special_tokens(question_input_ids,
                                                                    answer_input_ids if answer_length > 0 else None)
        token_type_ids = self.tokenizer.create_token_type_ids_from_sequences(question_input_ids, answer_input_ids)
    ...
    def _get_text(self, index):
        ...
        title, body, answer = [
            ...
            for txt, transform in zip([title, body, answer],
    ...
    def __getitem__(self, index):
        ...
        input_ids, attention_mask, token_type_ids = map(torch.LongTensor, [input_ids, attention_mask, token_type_ids])
...
class TestQuestDataset(QuestDataset):
    ...
    def __init__(self, data_df, tokenizer, max_seg_length=512, answer_ratio=0.5, title_ratio=0.5,
                 use_title=True, use_body=True, use_answer=True,
                 title_col='question_title', body_col='question_body', answer_col='answer'):
        super(TestQuestDataset, self).__init__(data_df=data_df, tokenizer=tokenizer, max_seg_length=max_seg_length,
                                               target_cols=None, answer_ratio=answer_ratio, title_ratio=title_ratio,
                                               use_title=use_title, use_body=use_body, use_answer=use_answer,
                                               title_col=title_col, body_col=body_col, answer_col=answer_col)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_path", type=Path, default="/kaggle/input/google-quest-challenge/")
    parser.add_argument("--model_dir", type=Path, default="/kaggle/input/stackx-80-aux-ep-3")
    ...
    def get_model(targets=ALL_TARGETS):
        config = BertConfig.from_json_file(args.model_dir / 'stackx-base-cased-config.json')
    ...
    tokenizer = BertTokenizer(args.model_dir / 'stackx-base-cased-vocab.txt', do_lower_case=False)
