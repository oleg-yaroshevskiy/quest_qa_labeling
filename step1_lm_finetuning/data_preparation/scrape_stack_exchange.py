...
def main():
    ...
    links = get_urls(main_url=MAIN_URL,
                     path_to_dump=PATH_TO_SX_DUMP)

    print(f"Fetched {len(links)} links to download data.")

    download_and_unzip_data(main_url=MAIN_URL,
                            links=links[1:SAMPLE],
                            path_to_dump=PATH_TO_SX_DUMP)

    parse_dumps(path_to_dump=PATH_TO_SX_DUMP,
                out_dir=OUT_DIR)
