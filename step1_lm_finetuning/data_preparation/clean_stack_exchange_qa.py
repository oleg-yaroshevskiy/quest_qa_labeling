...
def process(all_questions, all_answers, path_to_save, detect_lang=False,
            n_jobs=4, chunksize=100000):
    html_pattern = re.compile(r'<.*?>')

    question_body = all_questions['Body'].astype(str).progress_apply(
        lambda s: html_pattern.sub('', s))
    question_title = all_questions['Title'].astype(str).progress_apply(
        lambda s: html_pattern.sub('', s))
    ...
    question_features = all_questions[['Id', 'host', 'username', 'Score', 'ViewCount',
                                       'FavoriteCount', 'AnswerCount']]
    question_features = question_features.merge(question_ans_agg_features,
                                                left_on='Id', right_on='ParentId', how='left')
    question_features = pd.concat([question_features, question_title, question_body],
                                  axis=1, sort=False)
    ...
    if detect_lang:
        ...
        question_features = question_features.iloc[(all_questions['AnswerCount'].ravel() != 0)]

    question_features.columns = ['id', 'host', 'question_username',
                                 'question_score', 'question_views', 'question_favs',
                                 'answers_count', 'answers_max_score', 'answers_mean_score',
                                 'title', 'body']
    selected_answers = all_answers['Id'].isin(select_answers(all_answers))

    answer_features = all_answers[selected_answers][['ParentId', 'username', 'Body', 'Score',
                                                     'is_answer_accepted']]
    answer_features['Body'] = answer_features['Body'].astype(str).progress_apply(
        lambda s: html_pattern.sub('', s))

    answer_features.columns = ['ParentId', 'answer_username', 'answer', 'answer_score',
                               'is_answer_accepted']
    qa_features = question_features.merge(answer_features,
                                          left_on='id',
                                          right_on='ParentId',
                                          how='inner').drop(columns=['ParentId'])
    ...
    qa_features[targets] = np.sign(qa_features[targets]) * \
                           np.log1p(np.abs(qa_features[targets].astype('float')))
...
def select_answers(all_answers, max_answers_per_question=2):
    ...
    n_additional = max_answers_per_question - (answer_ids.groupby('ParentId')['is_answer_accepted'].sum() > 0).astype(
        int)
    ...
    while len(additional_answers) > 0:
        ...
        additional_answers = additional_answers.iloc[np.random.permutation(len(additional_answers))].reset_index(
            drop=True)
        selected_answers += additional_answers['Id'].loc[
            additional_answers['ParentId'].drop_duplicates().index].tolist()
